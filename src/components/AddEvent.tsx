...
export function AddEvent({ navigate, addEvent, editingEvent, isNewFromDashboard, userProfile }: AddEventProps) {
  // Mock user registration details
  const userRegistrationDetails = {
    collegeName: "Trinity College",
    address: '12, College Road, Bandra',
    locality: 'Bandra',
    city: 'Mumbai',
    pinCode: '400050'
  };
  ...
  useEffect(() => {
    if (editingEvent) {
      ...
      setEventData({
        ...
        address: editingEvent.address || userRegistrationDetails.address,
        locality: editingEvent.locality || userRegistrationDetails.locality,
        city: editingEvent.location?.split(', ')[1] || userRegistrationDetails.city,
        pinCode: userRegistrationDetails.pinCode,
  ...
  const handleSave = () => {
    ...
    const newEvent = {
      ...
      address: `${eventData.address}, ${eventData.locality}, ${eventData.city} ${eventData.pinCode}`,
      contact: organizingCommittee[0] ? `${organizingCommittee[0].name} • ${organizingCommittee[0].phone}` : 'Contact info not available',
    ...
    return (
      <div className="space-y-0.5">
        ...
        <div className="grid grid-cols-[85px_1fr] items-center">
          ...
          <div className="flex gap-1">
            <select
              ...
              onChange={(e) => updateSubEvent(subEvent.id, 'timeHour', e.target.value)}
            ...
            <select
              ...
              onChange={(e) => updateSubEvent(subEvent.id, 'timeMinute', e.target.value)}
            ...
            <select
              ...
              onChange={(e) => updateSubEvent(subEvent.id, 'timeAmPm', e.target.value)}
        ...
        <div className="grid grid-cols-[85px_1fr] items-center">
          ...
          <div className="relative" ref={entryFeesRef}>
            <input
              type="text"
              value={subEvent.entryFees || ''}
              onChange={(e) => {
                updateSubEvent(subEvent.id, 'entryFees', e.target.value);
                setEntryFeesSearch(prev => ({ ...prev, [subEvent.id]: e.target.value }));
                setShowEntryFeesDropdown({ [subEvent.id]: true });
              }}
              onFocus={() => setShowEntryFeesDropdown({ [subEvent.id]: true })}
              className={getInputClassName("w-full h-6 px-1 bg-transparent border border-gray-300 rounded text-sm text-black leading-none", currentErrors.entryFees)}
              placeholder="Rs. or Free"
            />
            {showEntryFeesDropdown[subEvent.id] && getFilteredEntryFeesOptions(subEvent.id).length > 0 && (
              <div className="absolute top-full left-0 w-full bg-white border border-gray-300 rounded mt-0.5 max-h-40 overflow-y-auto momentum-list-scroll inertial-scroll-y z-10 shadow-lg">
                {getFilteredEntryFeesOptions(subEvent.id).map(option => (
                  <div
                    key={option}
                    onClick={() => handleEntryFeesSelect(option, subEvent.id)}
                    className="px-1 py-1 text-sm text-black hover:bg-gray-100 cursor-pointer border-b border-gray-100 last:border-b-0"
                  >
                    {option}
                  </div>
                ))}
          ...
          <div className="flex gap-1 items-center justify-between">
            <div className="flex gap-1 items-center">
              <input
                ...
                onChange={(e) => updateSubEvent(subEvent.id, 'registrationDeadlineDate', e.target.value)}
                min={getCurrentDate()}
              ...
              <select
                ...
                onChange={(e) => updateSubEvent(subEvent.id, 'registrationDeadlineHour', e.target.value)}
              ...
              <select
                ...
                onChange={(e) => updateSubEvent(subEvent.id, 'registrationDeadlineMinute', e.target.value)}
            ...
            <select
              ...
              onChange={(e) => updateSubEvent(subEvent.id, 'registrationDeadlineAmPm', e.target.value)}
              ...
              <div key={index} className="space-y-0.5">
                ...
                <div className="grid grid-cols-2 grid-rows-2 gap-0.5">
                  ...
                  <input
                    ...
                    className="w-full h-6 px-1 bg-transparent border border-gray-300 rounded text-sm text-black leading-none placeholder-black"
                    placeholder={member.isFirstMember ? "" : "Name"}
                  ...
                  <select
                    ...
                    className={`w-full h-6 px-1 bg-transparent border border-gray-300 rounded text-sm leading-none text-left ${
                      member.designation ? 'text-black' : 'text-gray-400'
                    }`}
                    required
                  >
                  ...
                  <input
                    ...
                    className="w-full h-6 px-1 bg-transparent border border-gray-300 rounded text-sm text-black leading-none placeholder-black"
                    placeholder={member.isFirstMember ? "" : "Ph. No."}
                    disabled={member.isFirstMember}
                  />
                  {/* Row 2: Email */}
                  <input
                    ...
                    className="w-full h-6 px-1 bg-transparent border border-gray-300 rounded text-sm text-black leading-none placeholder-black"
                    placeholder={member.isFirstMember ? "" : "E-mail id"}
                    disabled={member.isFirstMember}
                  />
                </div>
