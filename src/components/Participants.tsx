...
export function Participants({ navigate, event, profileEmail, emailSent, participantData, selectedEventId }: ParticipantsProps) {
  ...
  // Get actual participants for this event
  const getEventParticipants = () => {
    ...
    // Iterate through all participant data to find registrations for this event
    Object.entries(participantData).forEach(([registrationKey, details]) => {
      ...
      if (registrationKey.startsWith(`${selectedEventId}:`)) {
        // Parse sub-event index from registration key (e.g., "pandu:0" -> index 0)
        const parts = registrationKey.split(':');
        if (parts.length === 2) {
          const subEventIndex = parseInt(parts[1]);
          const subEventName = event?.events?.[subEventIndex]?.type || `Sub Event ${subEventIndex + 1}`;
  ...
  const sendEmailManually = () => {
    ...
    const csvHeaders = ['S.No', 'Name', 'College', 'Phone', 'Email', 'Sub Event'];
    const csvData = actualParticipants.map((participant, index) => [
      index + 1,
      participant.name,
      participant.college,
      participant.phone,
...
- Total Participants: ${actualParticipants.length}
...
Zhevents Event Management System`;
  ...
  const sendWhatsAppMessages = () => {
    ...
    actualParticipants.forEach((participant, index) => {
    ...
    // Show success message
    alert(`Message will be sent to all ${actualParticipants.length} participants. WhatsApp chats will open automatically with your message pre-filled. Just click 'Send' in each chat!`);
  };
  ...
  const sendEmailToAllParticipants = () => {
    ...
    const participantEmails = actualParticipants.map(p => p.email).join(';');
    ...
    alert(`Email client opened with your email (${profileEmail}) as sender${ccMessage} and all ${actualParticipants.length} participants in BCC for privacy. Participants won't see each other's email addresses.`);
  };
  ...
  return (
    <div className="w-full min-h-screen bg-white p-4">
      ...
      <div className="mb-4">
        <div className="mb-3">
          <div className="text-sm text-black">Name • College Name • Phone No. • Email id • Sub Event</div>
        </div>
          ...
          <div className="bg-white rounded-lg p-6 max-w-sm w-full shadow-lg">
            ...
            <div className="mb-4">
              <p className="text-sm text-gray-600 mb-3 bg-blue-50 p-2 rounded">
                📱 This will automatically open {actualParticipants.length} WhatsApp chats with your message pre-filled. Just click 'Send' in each chat!
              </p>
      ...
      {showEmailModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 max-w-md w-full shadow-lg">
            ...
            <div className="mb-4">
              <p className="text-sm text-gray-600 mb-3 bg-blue-50 p-2 rounded">
                📧 This will open your email client with all {actualParticipants.length} participants in BCC (for privacy)
